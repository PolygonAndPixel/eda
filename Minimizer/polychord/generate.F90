...
module generate_module
    ...
    subroutine GenerateLivePoints(loglikelihood,prior,settings,RTI,mpi_information,context)
        use settings_module,  only: program_settings
        ...
        if(linear_mode(mpi_information)) then
            ...
            if(is_root(mpi_information)) then
                ...
                do while(.true.)
                    ...
                    time0 = time()
                    call calculate_point( loglikelihood, prior, live_point, settings,nlike,context) ! Compute physical coordinates, likelihoods and derived parameters
                    time1 = time()
        ...
        call time_speeds(loglikelihood,prior,settings,RTI,speed,mpi_information,context)
    ...
    subroutine time_speeds(loglikelihood,prior,settings,RTI,speed,mpi_information,context)
        use settings_module,  only: program_settings
        use run_time_module,   only: run_time_info
        use random_module,   only: random_reals
    ...
    subroutine GenerateLivePointsFromSeed(loglikelihood,prior,settings,RTI,&
