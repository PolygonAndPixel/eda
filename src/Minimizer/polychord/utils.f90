module utils_module
    ...
    function calc_covmat(lives,phantoms) result(covmat)
        ...
        covmat = matmul(lives    - spread(mean,dim=2,ncopies=nlive) ,    &
            transpose(lives    - spread(mean,dim=2,ncopies=nlive)    ) ) &
            +    matmul(phantoms - spread(mean,dim=2,ncopies=nphantom) , &
            transpose(phantoms - spread(mean,dim=2,ncopies=nphantom) ) )
    ...
    function calc_similarity_matrix(data_array) result(similarity_matrix)
        ...
        similarity_matrix = similarity_matrix + transpose(similarity_matrix) - &
            2d0 * matmul( transpose(data_array),data_array )
