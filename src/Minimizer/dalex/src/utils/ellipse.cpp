...
index_t ellipse::contains(const array_1d<value_t> &pt){
    return contains(pt, 0);
}

index_t ellipse::contains(const array_1d<value_t> &pt, const index_t use_extreme){
    index_t i,j;
    value_t sum=0.0;
    value_t component,threshold;
...
void ellipse::build(const array_2d<value_t> &pts_in){

    array_1d<value_t> dummy_center;
    build(dummy_center, pts_in);

}

void ellipse::build(const array_1d<value_t> &center_in,
                    const array_2d<value_t> &pts_in){
    ...
    index_t n_pts = pts_in.get_rows();
    index_t dim = pts_in.get_cols();
...
void ellipse::_find_center(const array_2d<value_t> &pts_in){
    index_t n_pts=pts_in.get_rows();
    index_t dim=pts_in.get_cols();
...
void ellipse::_set_radii(const array_2d<value_t> &pts_in){

    array_1d<index_t> bad_dexes;
    index_t i;
    for(i=0;i<pts_in.get_rows();i++){
        if(contains(pts_in(i))==0){
            bad_dexes.add(i);
    ...
    index_t ipt;
    array_1d<value_t> norm,norm_sorted;
    array_1d<index_t> norm_dex,norm_ct;
    ...
    array_1d<index_t> norm_ct_sorted;
